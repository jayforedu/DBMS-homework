...
| `name`         | INT (PK)     | 僅可包含英文大小寫與數字（A-Z, a-z, 0-9），不得有空格或特殊字元，長度 3–20 字元。正規表示式：`^[A-Za-z0-9]{3,20}$`。                |
| `email`        | INT (PK)     | 唯一且不可為空，需符合有效 Email 格式，例如 `user@example.com`，不可含空格。可使用正規表示式 `^[\w\.-]+@[\w\.-]+\.\w{2,}$` 驗證。 |
| `phone_number` | INT (PK)     | 不可為空。必須為 10 位數字且開頭為 `09`。正規表示式：`^09\d{8}$`。                                                   |
| `gender`       | INT (PK)     | 不可為空。僅允許「男」、「女」、「其他」三種值。建議使用 ENUM 或 CHECK 限制。                                                 |
| `birthdate`    | INT (PK)     | 賣家生日， 不可為空。格式須為`YYYY-MM-DD`，必須為有效歷史日期，不得晚於系統當前日期。                                                  |
